/** 游戏视图控制器，负责游戏场景切换 */
var __reflect = (this && this.__reflect) || function (p, c, t) {
    p.__class__ = c, t ? t.push(c) : t = [c], p.__types__ = p.__types__ ? t.concat(p.__types__) : t;
};
var __extends = (this && this.__extends) || (function () {
    var extendStatics = Object.setPrototypeOf ||
        ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
        function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var GameView = (function (_super) {
    __extends(GameView, _super);
    function GameView() {
        return _super.call(this) || this;
    }
    GameView.prototype.start = function () {
        this.chanageScene(new LoginScene());
    };
    GameView.prototype.chanageScene = function (scene, data) {
        if (scene == null) {
            throw ("scene is null");
        }
        if (this.curScene == scene) {
            return;
        }
        if (this.curScene == null || this.curScene.bgName != scene.bgName) {
            var newBg = scene.createBg();
            this.addChild(newBg);
            if (this.curSceneBg != null) {
                this.removeChild(this.curSceneBg);
            }
            this.curSceneBg = newBg;
        }
        this.addChild(scene);
        if (this.curScene != null) {
            this.removeChild(this.curScene);
            this.addRemovedScene(this.curScene);
        }
        this.curScene = scene;
        this.curScene.init(this, data);
    };
    GameView.prototype.addRemovedScene = function (scene) {
    };
    GameView.prototype.addRemovedSceneBg = function (bg) {
    };
    return GameView;
}(egret.DisplayObjectContainer));
__reflect(GameView.prototype, "GameView", ["IGameView"]);
//# sourceMappingURL=GameView.js.map