...
class SkinnedMesh implements egret3d.framework.IUserCode
{
        ...
        this.app.getAssetMgr().loadCompressBundle(shaderAssetUrl, (state) => {
            if (state.isfinish) {
                ...
                this.app.getAssetMgr().load(baihuAssetUrl, egret3d.framework.AssetTypeEnum.Auto, (s) => {
                    if (s.isfinish) {
                        let prefab: egret3d.framework.Prefab = this.app.getAssetMgr().getAssetByName("elong.prefab.json") as egret3d.framework.Prefab;
                        ...
                        // 动画控制
                        let ap = this.elong.gameObject.getComponent(egret3d.framework.StringUtil.COMPONENT_ANIPLAYER) as egret3d.framework.AniPlayer;
                        ap.playByIndex(0);
                    }
                });
            }
    ...
    private lookAtPoint: egret3d.math.Vector3 = new egret3d.math.Vector3(0, 2.5, 0);
